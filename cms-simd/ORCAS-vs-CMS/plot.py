...
def plot_speed_and_error(alpha:float, fixed_orcas:int, fixed_cms:int): # ORCAS array counter, CMS row number
    ...
    for sketch in SKETCHES:
        ...
        if sketch == ORCAS:
            ...
            speed_x = [fixed_cms*width*4/1024 for (width, speed) in speed_dict[sketch]]
            speed_y = [speed for (width, speed) in speed_dict[sketch]]
            speed_ax.plot(speed_x, speed_y, label=str(sketch))

            error_x = [fixed_cms*width*4/1024 for (width, l2) in error_dict[sketch]]
            error_y = [l2 for (width, l2) in error_dict[sketch]]
            error_ax.plot(error_x, error_y, label=str(sketch))

    N = speed_data[ORCAS][0].split('\t')[1]
    fig_title = 'N = ' + N + ' | alpha = ' + str(alpha) + ' | ORCAS array counter = ' + str(fixed_orcas)  + ' | CMS row number = ' + str(fixed_cms)
...
def plot_all_alpha():
    ...
    plot_speed_and_error(1.0, fixed_orcas=3, fixed_cms=4)
    plot_speed_and_error(1.0, fixed_orcas=4, fixed_cms=4)
    plot_speed_and_error(1.0, fixed_orcas=3, fixed_cms=1)
    plot_speed_and_error(1.0, fixed_orcas=4, fixed_cms=1)
    plot_speed_and_error(1.0, fixed_orcas=3, fixed_cms=10)
    plot_speed_and_error(1.0, fixed_orcas=4, fixed_cms=10)
    plot_speed_and_error(1.0, fixed_orcas=3, fixed_cms=2)
    plot_speed_and_error(1.0, fixed_orcas=4, fixed_cms=2)
