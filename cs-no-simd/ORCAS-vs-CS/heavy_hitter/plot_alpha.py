...
def get_all_values(hh_data, fixed_orcas, fixed_cs, fixed_sketch_size, fixed_threshold):
...
def plot_speed_and_error(fixed_orcas:int, fixed_cs:int, fixed_sketch_size=None, fixed_threshold=None): # ORCAS array counters, CS row numbers
    ...
    for alpha in ALPHA:
        ...
        for fn in os.listdir(alpha_folder_name):
            indiv_data = open(alpha_folder_name + fn).readlines()
            if 'final_error' in fn:
                hh_data[CS][alpha] = hh_data[CS][alpha] + indiv_data

    hh_dict_all = get_all_values(hh_data, fixed_orcas, fixed_cs, fixed_sketch_size, fixed_threshold)
    ...
    fig_title = 'N = ' + N + ' | ORCAS config: array counter | CS config: row number'
    ...
    hh_ax.legend(loc="upper right", prop={"size":20}, ncol=len([key for key in hh_dict]), bbox_to_anchor=(1.0, 1.14))

def plot_all_alpha():

    plot_speed_and_error(fixed_orcas=[1,3,5], fixed_cs=[1], fixed_sketch_size=65536, fixed_threshold=0.000316)
