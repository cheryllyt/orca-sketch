...
def get_all_values(hh_data, fixed_orcas, fixed_cus, fixed_sketch_size, fixed_threshold):
...
def plot_speed_and_error(fixed_orcas:int, fixed_cus:int, fixed_sketch_size=None, fixed_threshold=None): # ORCAS array counters, CUS row numbers
    ...
    for alpha in ALPHA:
        ...
        for fn in os.listdir(alpha_folder_name):
            indiv_data = open(alpha_folder_name + fn).readlines()
            if 'final_error' in fn:
                hh_data[CUS][alpha] = hh_data[CUS][alpha] + indiv_data

    hh_dict_all = get_all_values(hh_data, fixed_orcas, fixed_cus, fixed_sketch_size, fixed_threshold)
    ...
    fig_title = 'N = ' + N + ' | ORCAS config: array counter | CUS config: row number'
...
def plot_all_alpha():
    ...
    plot_speed_and_error(fixed_orcas=[3,4], fixed_cus=[1,2,4], fixed_sketch_size=65536, fixed_threshold=0.000316)
