...
void ORCASketch::initialize(int sketch_size, int number_of_buckets, int number_of_bucket_counters, int seed)
{
    ...
    {
        int temp[bucket_size];
        _mm256_storeu_si256((__m256i*) temp, bucket_counter_inv_lookup_table[i]);
        for (int j = 0; j < bucket_size; j++)
        {
            cout << temp[j] << " ";
        }
...
void ORCASketch::increment(const char * str)
{
    ...
    uint32_t min_inc = min + 1;
    __m256i min_inc_vec = _mm256_set_epi32(min_inc, min_inc, min_inc, min_inc, min_inc, min_inc, min_inc, min_inc);

    __m256i inv_counter_vec = bucket_counter_inv_lookup_table[option_index];
    __m256i min_inc_counter_vec = _mm256_and_si256(min_inc_vec, inv_counter_vec);

    __m256i& orca_ptr = *((__m256i*) &orca_sketch[exact_bucket_index]);
    orca_ptr = _mm256_max_epi32(orca_ptr, min_inc_counter_vec);

    #ifdef DEBUG
    cout << "min: " << min << "\n";
    cout << "min_inc: " << min_inc << "\n";
    cout << "min_inc_vec: ";
    int temp_min_inc_vec[bucket_size];
    _mm256_storeu_si256((__m256i*) temp_min_inc_vec, min_inc_vec);
    for (int j = 0; j < bucket_size; j++)
    {
        cout << temp_min_inc_vec[j] << " ";
    }
    cout << "\ninv_counter_vec: ";
    int temp_counter_vec[bucket_size];
    _mm256_storeu_si256((__m256i*) temp_counter_vec, inv_counter_vec);
    for (int j = 0; j < bucket_size; j++)
    {
        cout << temp_counter_vec[j] << " ";
    }
    cout << "\nmin_cou_vec: ";
    int temp_min_inc_counter_vec[bucket_size];
    _mm256_storeu_si256((__m256i*) temp_min_inc_counter_vec, min_inc_counter_vec);
    for (int j = 0; j < bucket_size; j++)
    {
        cout << temp_min_inc_counter_vec[j] << " ";
    }
...
void ORCASketch::create_lookup_tables()
{
    ...
    bucket_counter_inv_lookup_table = new __m256i[number_of_options];
    ...
    {
        int temp[bucket_size];
        int inv_temp[bucket_size];
        for (int j = 0; j < bucket_size; j++)
        {
            num = f.get();
            int_num = num - CHAR_TO_INT_DIFF;
            temp[j] = int_num;
            inv_temp[j] = int_num == 0 ? 0 : -1;
        }
        ...
        bucket_counter_inv_lookup_table[i] = _mm256_set_epi32(inv_temp[7], inv_temp[6], inv_temp[5], inv_temp[4],
                                                              inv_temp[3], inv_temp[2], inv_temp[1], inv_temp[0]);
    }
